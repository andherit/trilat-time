...
!          cycle if the node kn is not attached to pmin%idnode
           if (amesh%FVsNodes(pmin%idnode,kn,1)==0) cycle
...
! Wavefront has crossed from one velocity layer to another 
               if (tri_ops%k1 <= epsilon(1.).or.tri_ops%k2 <= epsilon(1.)) then 
...
! planar operator                
               if (do_tplane) call calc_tplane(tri_ops,tplane(i))
                if (verbose) then 
                  write(*,*) '---------------------------------------'
...
!       WORSE move to the next node attached to pmin%inode           
           if (ttest-secondary_time(kn) > water_level(ttest)) cycle
...
!       order of preferences : tface(),thead,tcface,tedge,tplane()
!       modes :                  1       2      3     4      5     
...
!        call test_head(ttest,thead,khead,time(kn),mode(kn),kappa(kn),gonextkn,ttestmatch)
        do i=1,2
...
!       equal but with a better mode
        call test_edge(ttest,tedge,kedge,secondary_time(kn),mode(kn),kappa(kn),gonextkn,ttestmatch)
        if (gonextkn) cycle 
...
!###############################################################################
subroutine test_face(ttest,tface,kface,time,mode,kappa,gonextkn,ttestmatch)
...
if (abs(ttest-tface) <= water_level(ttest)) then
   !       equal but with a better mode
   ...
   !       better
      ...
      mode = 1
      kappa = kface
      ! if (kface > 1500._pr )  kappa = infinity
      time = tface
   endif
   ttestmatch=.true.
   ! exit
...
!###############################################################################
subroutine test_head(ttest,thead,khead,time,mode,kappa,gonextkn,ttestmatch)
...
!       thead is the best (mode=2)
!       equal but with a better mode
if (abs(ttest-thead) <= water_level(ttest).and..not.ttestmatch) then
...
return
end subroutine test_head 
!###############################################################################
subroutine test_edge(ttest,tedge,kedge,time,mode,kappa,gonextkn,ttestmatch)
...
!       tedge is the best (mode=4)
!       equal but with a better mode
if (abs(ttest-tedge) <= water_level(ttest).and..not.ttestmatch) then
...
!###############################################################################
subroutine test_plane(ttest,tplane,time,mode,kappa,gonextkn,ttestmatch)
...
if (abs(ttest-tplane) <= water_level(ttest).and..not.ttestmatch) then
!       equal but with a better mode
...
subroutine calc_thead(tri_ops,r3,t3)
   ...
   cos_c1m = (-tri_ops%x3*tri_ops%xc2-tri_ops%y3*tri_ops%yc2)     &
              /sqrt(tri_ops%xc2**2._pr+tri_ops%yc2**2._pr)        &
...
!###############################################################################
subroutine calc_tplane(tri_ops,t3)
   ...
   ! if ( ( tri_ops%t2 <= tri_ops%t1+tri_ops%d12*tri_ops%x3/(tri_ops%v1*sqrt(tri_ops%x3**2._pr+tri_ops%y3**2._pr))  ).and.       &
      !   (tri_ops%t2 >= tri_ops%t1-tri_ops%d12*(tri_ops%d12-tri_ops%x3)/(tri_ops%v1*sqrt((tri_ops%d12-tri_ops%x3)**2._pr+(tri_ops%y3)**2._pr)) ) ) then 
              ! wave propagating from left to right
         !  t3 = ta + tri_ops%y3*sqrt( (1._pr/tri_ops%v1)**2._pr - ((tri_ops%t1-tri_ops%t2)/tri_ops%d12)**2._pr ) ! 
